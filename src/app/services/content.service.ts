...
import { IRequest, IRequestItem } from '../models';
import { RemoteService } from './remote.service';
import { ContainerComponent } from '../ui-containers/components';
import { IValue } from '../ui-containers/models';
...
export class ContentService {
  ...
  /**
   * updateItem is called by every connected UI element
   * informing global content service of new application state
   *
   * @param path   // components this.path
  ...
  private _buildChildContainerPath(container: ContainerComponent, containerid = '00'): IContainerData[] {
    ...
    container.children.forEach((childContainer: ContainerComponent) => {
      ...
      setContainerPath(childContainer, container.path, this._branchId++);
    ...
    function setContainerPath(container: ContainerComponent, prefix: string, id: number ) {
      ...
      function getPrefix(parentPrefix: string, name: string, title: string) {
        const containerName = (name
          ? name
          : ((title || '').replace(/ |\./g, '_')) || 'b') + ('_' + id);

        return `${ parentPrefix || '' }${ parentPrefix ? '.' : '' }${ containerName }`;
      }
    }
  }
